...
def read_data():
    df = spark.sql("SELECT imei_id, lgt, ltt, ts FROM parquet.`hdfs://192.168.2.3:9000/shanghai.parquet`")
    return df
...
def cal_region_geohash(df_region):
    df_region = df_region.withColumn('geohash5', geohashUDF(df_region['latitude'],df_region['longitude']))
    df_region = df_region.withColumn('g6', geohashUDF6(df_region['latitude'],df_region['longitude']))
...
df_region = cal_region_geohash(df_region)
df_region = df_region.withColumn('abc',lit("ABC"))
w = Window().partitionBy('abc').orderBy('geohash5')
