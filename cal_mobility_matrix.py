...
def read_data():
    df_1 = spark.sql("SELECT * FROM parquet.`hdfs://master:9000/user/honlan/shanghai_region_id6.parquet` where date=='2019-07-01'")
    df_2 = spark.sql("SELECT * FROM parquet.`hdfs://master:9000/user/honlan/shanghai_region_id6.parquet` where date=='2019-07-09'")
    df_shanghai = spark.sql("SELECT * FROM parquet.`hdfs://master:9000/user/honlan/shanghai_region_id6.parquet` where date>='2019-07-02' and date<='2019-07-08'")
...
def _map_row_region(row):
    return (str(row[1])+';'+row[2], 1)

def _map_mobility_matrix(row):
    pre_agent_id = str(row[4])
    return (pre_agent_id+';'+str(row[1])+';'+row[3], 1)
...
for i in range(23, 191):
    ...
    mobility_dict[mobility_raw_matrix[i][0]] = csr_matrix((data, (rows, cols)), shape=(356, 356))
...
for k in mobility_dict:
    ...
    mobility_frac_dict[k] = _tmp / repmat(ppl_region_dict[k], 356, 1)
    mobility_frac_dict[k] = np.nan_to_num(mobility_frac_dict[k])
    mobility_frac_dict[k] = csr_matrix(mobility_frac_dict[k])

with open('/data/ppl_region5_max.pkl', 'wb') as wf:
    pickle.dump(ppl_region_dict, wf)

with open('/data/mobility_frac_matrix_max.pkl', 'wb') as wf:
    pickle.dump(mobility_frac_dict, wf)

with open('/data/mobility_matrix_max5.pkl', 'wb') as wf:
    pickle.dump(mobility_dict, wf)